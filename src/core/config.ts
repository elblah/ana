...
export class Config {
  ...
  static get temperature(): number {
    ...
    return temp ? parseFloat(temp) : 0.0;
  }

  static get maxTokens(): number | null {
    const maxTokens = process.env.MAX_TOKENS;
    return maxTokens ? parseInt(maxTokens, 10) : null;
  }

  // Streaming Configuration
  static get streamingTimeout(): number {
    return parseInt(process.env.STREAMING_TIMEOUT || '300', 10);
  }

  static get streamingReadTimeout(): number {
    return parseInt(process.env.STREAMING_READ_TIMEOUT || '30', 10);
  }

  static get totalTimeout(): number {
    return parseInt(process.env.TOTAL_TIMEOUT || '300', 10) * 1000;
  }

  // Context and Memory Configuration
  static get contextSize(): number {
    return parseInt(process.env.CONTEXT_SIZE || '128000', 10);
  }

  static get contextCompactPercentage(): number {
    return parseInt(process.env.CONTEXT_COMPACT_PERCENTAGE || '0', 10);
  }
  ...
  static get compactProtectRounds(): number {
    return parseInt(process.env.COMPACT_PROTECT_ROUNDS || '2', 10);
  }

  static get minSummaryLength(): number {
    return parseInt(process.env.MIN_SUMMARY_LENGTH || '100', 10);
  }

  static get forceCompactSize(): number {
    return parseInt(process.env.FORCE_COMPACT_SIZE || '5', 10);
  }

  // Tool Configuration
  static get maxToolResultSize(): number {
    return parseInt(process.env.MAX_TOOL_RESULT_SIZE || '300000', 10);
  }

  // YOLO mode state
  private static _yoloMode: boolean = false;
  ...
  static setSandboxDisabled(disabled: boolean): void {
    ...
    if (disabled) {
      console.log(`${this.colors.yellow}[*] Sandbox temporarily disabled for this session${this.colors.reset}`);
    } else {
      console.log(`${this.colors.green}[*] Sandbox re-enabled for this session${this.colors.reset}`);
    }
  }
  ...
  static validateConfig(): void {
    if (!this.baseUrl) {
      console.error(`${this.colors.red}Error: Missing required environment variable:${this.colors.reset}`);
      console.error(`${this.colors.red}  - API_BASE_URL or OPENAI_BASE_URL${this.colors.reset}`);
      console.error(`${this.colors.reset}`);
      console.error(`${this.colors.cyan}Example configuration:${this.colors.reset}`);
      console.error(`${this.colors.cyan}  export API_BASE_URL="https://your-api-provider.com/v1"${this.colors.reset}`);
      console.error(`${this.colors.reset}`);
      console.error(`${this.colors.yellow}Optional variables:${this.colors.reset}`);
      console.error(`${this.colors.yellow}  export API_KEY="your-api-key-here" (optional, some providers don't require it)${this.colors.reset}`);
      console.error(`${this.colors.yellow}  export API_MODEL="your-model-name" (optional, some providers have a default)${this.colors.reset}`);
  ...
  static printStartupInfo(): void {
    ...
    if (process.env.TEMPERATURE) {
      console.log(`${this.colors.green}  Temperature: ${this.temperature}${this.colors.reset}`);
    }
