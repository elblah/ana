...
export class CompactionService {
  ...
  async forceCompactRounds(messages: Message[], n: number): Promise<Message[]> {
    ...
    const filteredMessages = messagesToCompact.filter(msg => !msg.content?.startsWith('[SUMMARY]'));
    
    if (filteredMessages.length === 0) {
      return messages;
    }

    const summary = await this.getAISummary(filteredMessages.map(msg => ({ messages: [msg], isSummary: false, isUserTurn: false })));
    const summaryMessage = this.createSummaryMessage(summary);
  ...
  async forceCompactMessages(messages: Message[], n: number): Promise<Message[]> {
    ...
    const summary = await this.getAISummary(messagesToCompact.map(msg => ({ messages: [msg], isSummary: false, isUserTurn: false })));
    const summaryMessage = this.createSummaryMessage(summary);
  ...
  private groupMessages(messages: Message[]): MessageGroup[] {
    ...
    for (const msg of messages) {
      ...
      if (msg.role === 'tool' ||
          (msg.role === 'user' && current.length > 1)) {
        groups.push({
          messages: [...current],
...
Your summary should be comprehensive enough to provide context but concise enough to be quickly understood.`
      ...
      ], false, true);  // false = non-streaming, true = throwOnError
      ...
      if (!this.validateSummary(summary)) {
        console.log(`${Config.colors.yellow}[!] Generated summary appears too short, using anyway${Config.colors.reset}`);
      }
    ...
    return messages.map((msg, index) => {
      ...
      if (role === 'assistant') {
        ...
        if (toolCalls && toolCalls.length > 0) {
          const toolInfo = toolCalls.map((call: any) => {
        ...
        if (toolContent.length > 500) {
          toolContent = toolContent.substring(0, 500) + '... (truncated for summarization)';
        }
   ...
   */
  private replaceMessagesWithSummary(messages: Message[], toReplace: Message[], summary: Message): Message[] {
    if (toReplace.length === 0) return messages;

    // Find indices by matching content and role (more reliable than object reference)
    const firstIndex = messages.findIndex(msg => 
      msg.role === toReplace[0].role && msg.content === toReplace[0].content
    );
    ...
    const lastIndex = messages.findIndex((msg, idx) => 
      idx >= firstIndex &&
    ...
    return [
      ...
      summary,
      ...messages.slice(actualLastIndex + 1)
    ];
  }
}