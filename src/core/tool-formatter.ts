...
export interface ToolOutput {
  ...
  important?: {       // Structured data when detail mode is ON
    [key: string]: any;
  };
  detailed?: {
    [key: string]: any;
...
export class ToolFormatter {
  ...
  static formatPreview(preview: ToolPreview): string {
    ...
    if (preview.warning) {
      lines.push(`${Config.colors.yellow}[!] Warning: ${preview.warning}${Config.colors.reset}`);
    }
  ...
  static formatForAI(output: ToolOutput): string {
    ...
    if (output.results?.content && lines.length > 0) {
      lines.push('');
      lines.push(output.results.content);
    }
   ...
   */
  private static formatValueForAI(value: any): string {
