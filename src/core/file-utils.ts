...
export class FileUtils {
   ...
   */
  static checkSandbox(path: string, context: string = 'file operation'): boolean {
    ...
    const currentDirWithSlash = this.currentDir.endsWith('/') ? this.currentDir : this.currentDir + '/';
    ...
    if (!(resolvedPath === this.currentDir || resolvedPath.startsWith(currentDirWithSlash))) {
      console.log(`${Config.colors.yellow}[x] Sandbox: ${context} trying to access "${resolvedPath}" outside current directory "${this.currentDir}"${Config.colors.reset}`);
      return false;
    }

    // Also check for obvious parent directory traversal
    if (path.includes('../')) {
      console.log(`${Config.colors.yellow}[x] Sandbox: ${context} trying to access "${path}" (contains parent traversal)${Config.colors.reset}`);
      return false;
    }
  ...
  static async listDirectory(path: string): Promise<string[]> {
    ...
    if (!this.checkSandbox(resolvedPath, 'list_directory')) {
      throw new Error(`list_directory: path "${path}" (resolves to "${resolvedPath}") outside current directory not allowed`);
    }

    try {
      const { promises: { readdir, stat } } = await import('node:fs');
      ...
      const validEntries = entries.filter(entry => {
        ...
        return false;
      }).filter(entry => !['node_modules', '.git', '.vscode', '.idea', 'dist', 'build'].includes(entry.name));
