...
export class MemoryCommand extends BaseCommand {
  ...
  async execute(args: string[]): Promise<CommandResult> {
    if (!process.env.TMUX) {
      console.log(`${Config.colors.red}This command only works inside a tmux environment.${Config.colors.reset}`);
      console.log(`${Config.colors.yellow}Please run this command inside tmux.${Config.colors.reset}`);
    ...
    try {
      ...
      console.log(`${Config.colors.cyan}Exported ${messages.length} messages to ${tempFile}${Config.colors.reset}`);

      console.log(`${Config.colors.cyan}Opening ${editor} in tmux window...${Config.colors.reset}`);
      console.log(`${Config.colors.dim}Save and exit when done. The editor is running in a separate tmux window.${Config.colors.reset}`);
      ...
      if (!file.exists()) {
        console.log(`${Config.colors.red}Session file not found after editing${Config.colors.reset}`);
        return { shouldQuit: false, runApiCall: false };
      }
        ...
        for (const msg of editedMessages) {
          switch (msg.role) {
            ...
            case 'tool':
              ...
              break;
            default:
              console.log(`${Config.colors.yellow}Warning: Unknown message role '${msg.role}', treating as user${Config.colors.reset}`);
              this.context.messageHistory.addUserMessage(msg.content || '');
          }
        }

        console.log(`${Config.colors.green}Reloaded ${editedMessages.length} messages from editor${Config.colors.reset}`);
      } else {
