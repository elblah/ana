...
export const TOOL_DEFINITION = {
  ...
  hide_results: false,
  description: 'Lists the contents of a specified directory recursively. Uses native Node.js functions. Defaults to current directory.',
  parameters: {
    type: 'object',
    properties: {
      path: {
  ...
  formatArguments: (args: ListDirectoryParams): string => {
...
export async function executeListDirectory(
  ...
  try {
    ...
    if (!(FileUtils as any).checkSandbox(resolvedPath, 'list_directory')) {
    ...
    stats.addToolTime(parseFloat(duration));
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          if (entryStat.isFile()) {
            ...
            if (!entry.startsWith('.') && !['node_modules', '.git', '.vscode', '.idea', 'dist', 'build'].includes(entry)) {
