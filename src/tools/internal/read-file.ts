...
export const TOOL_DEFINITION = {
  ...
  hide_results: false,
  description: 'Reads the content from a specified file path. Supports pagination with offset and limit parameters.',
  parameters: {
    type: 'object',
    properties: {
      path: {
  ...
  formatArguments: (args: ReadFileParams): string => {
    const { path, offset = 0, limit = DEFAULT_READ_LIMIT } = args;
    const lines: string[] = [];
  ...
  try {
    ...
    const truncatedLines = selectedLines.map(line => {
      if (line.length > MAX_LINE_LENGTH) {
        return line.slice(0, MAX_LINE_LENGTH) + `... [truncated, total: ${line.length} chars]`;
      }
      return line;
    });
